...
//                postorder traversals, search, and node count.
//
...
{
    private Node root;
    private int countStatic = 0;  // Used for Count method to mimic static behavior in C
    ...
    public int count()
    {
        countStatic = 0;       // Reset count before recursion
        countHelper(root);
        return countStatic;
    }
    ...
    private void countHelper(Node node)
    {
        ...
        {
            countStatic++;
            countHelper(node.rchild); // Important: rchild first as in C code
            countHelper(node.lchild);
        }
    }
}

public class Program498
{
