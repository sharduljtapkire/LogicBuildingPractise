...
//                Supports commands: exit, help, clear, man, creat, unlink, ls
//                Implements file creation and deletion simulation.
...
        System.out.println("ls : It is used to list all existing files");
...
        {
            System.out.println("Description : This command is used to list all existing files in the file system");
...
    //
    ///////////////////////////////////////////////////////////////////////
    public static Inode GetInode(String name) // Name of file that we want to check
    {
        Inode temp = head;
        while (temp != null)
        {
            // Check if the filename matches AND it's a regular, active file
            if (!temp.FileName.isEmpty() && temp.FileName.equals(name) && (temp.FileType == REGULARFILE))
            {
                return temp;
            }
            temp = temp.next;
...
    }
    
    // Wrapper function for simple boolean check
    public static boolean IsFileExists(String name)
    {
        return GetInode(name) != null;
    }
...
        // Loop to search free Inode (FileType == 0)
        while (temp != null)
        {
            if (temp.FileType == 0)
            {
...
        // Search first empty slot from UFDT (File Descriptor Table)
...
            return -1; // This should ideally be a dedicated error, but -1 is used
        }
...
        return i; // Return file descriptor (FD)
    }
...
        Inode inodeToUnlink = GetInode(name);
        if (inodeToUnlink == null)
        {
...
                {
                    tempInode = uareaobj.UFDT[i].ptrinode;

                    // Deallocate file table (by setting to null for GC)
                    uareaobj.UFDT[i] = null;
                    isFileOpen = true;
                    break; // File is deleted from OFT/UFDT, now clean up Inode
                }
            }
        }

        // 2. If it was not open, get the Inode directly for cleanup
        if (tempInode == null)
        {
            tempInode = inodeToUnlink;
        }

        // 3. Clean up the Inode
        // Deallocate the memory of buffer
        tempInode.Buffer = null;

        // Reset all values of Inode
        tempInode.FileName = ""; // Important
        tempInode.FileSize = 0;
        tempInode.ActualFileSize = 0;
        tempInode.LinkCount = 0;
        tempInode.Permission = 0;
        tempInode.FileType = 0; // Mark as FREE
        tempInode.ReferenceCount = 0;
...
        while (temp != null)
        {
            // Only display files that are marked as regular files (FileType == 1)
